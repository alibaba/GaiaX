...
class GXComponentViewTest : GXBaseTest() {
    ...
    fun template_border() {
        ...
        Assert.assertEquals(
            true,
            rootView.child(0).background is GradientDrawable
        )
        val bg = rootView.child(0).background as GradientDrawable
        Assert.assertEquals(
            1F.dpToPx(),
            bg.getStrokePaint().strokeWidth
        )
        Assert.assertEquals(
            Color.parseColor("#e61616"),
            bg.getStrokePaint().color
        )
    }
    ...
    fun template_animation_lottie_local_2() {
        val templateItem =
            GXTemplateEngine.GXTemplateItem(
                GXMockUtils.context,
                "view",
                "template_animation_lottie_local_2"
            )
        val templateData = GXTemplateEngine.GXTemplateData(JSONObject())
        val rootView = GXTemplateEngine.instance.createView(templateItem, size)
        GXTemplateEngine.instance.bindData(rootView, templateData)
